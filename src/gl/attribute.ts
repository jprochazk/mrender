...
const Uint32ElementInfo = {
  elementSize: 4,
  type: WEBGL.UNSIGNED_INT,
  isFloat: false,
};
const Uint16ElementInfo = {
  elementSize: 2,
  type: WEBGL.UNSIGNED_SHORT,
  isFloat: false,
};
const Uint8ElementInfo = {
  elementSize: 1,
  type: WEBGL.UNSIGNED_BYTE,
  isFloat: false,
};
export function typeinfo(array: TypedArray): {
  elementSize: number;
  type: number;
  isFloat: boolean;
} {
  ...
  throw new Error(
    `Element size of type ${(<any>array).constructor.name} is unknown`
  );
}
