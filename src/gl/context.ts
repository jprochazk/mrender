...
export class Context {
  ...
  createTexture(options?: TextureOptions): Texture {
    return new Texture(this.guidSequence++, this.gl, options);
  }

  draw(vertices: number, mode: number = WEBGL.TRIANGLES, offset: number = 0): this {
  ...
  drawIndexed(
    indices: number,
    type: number = WEBGL.UNSIGNED_SHORT,
    mode: number = WEBGL.TRIANGLES,
    offset: number = 0
  ): this {
